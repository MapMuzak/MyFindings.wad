...
`Actor.material.bin` \
disable most entities (horses excluded), decorated pots, conduits, chests, shulker boxes, copper golem, signs, hanging signs, piston head, bell, enchanting table book\
`ActorBanner.material.bin` \
disable only banners\
`ActorGlint.material.bin` \
disable enchanted armor, splash potions
...
disable blocks, fog (water, lava, etc)
...
disable [interaction box](https://minecraft.wiki/w/Hitbox#Interaction_box) only if outline selection is disabled

`Cracks.material.bin` \
`CracksBlockEntity.material.bin` \
disable [breaking](https://minecraft.wiki/w/Breaking) progress
